...
- For tests, helper flows, or ad hoc verification setups that spawn background services, daemons, browsers, temporary clones, patch files, or other agent-owned temporary resources, verify teardown and remove them before concluding; if cleanup fails, fix the harness or cleanup path instead of leaving residue.
