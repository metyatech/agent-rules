...
- When launching `agent-browser` on Windows, do not rely on the implicit default session; always provide an explicit `--session <name>` or set `AGENT_BROWSER_SESSION` to a chosen session name before use.
- If `agent-browser` fails with a localhost bind/access error (for example `os error 10013`), retry with a different explicit session name instead of reusing the blocked default session, and report the working session name or the remaining failure.
