...
- User-facing tools and web apps must be understandable from the product UI itself without requiring a separate chat explanation to use the primary flows.
- For user-facing tools and web apps, include in-product guidance for core tasks: what the tool does, what inputs mean, what outputs mean, and how to start with a sensible first use.
- Do not rely on README or final chat responses as the primary usage explanation for end users when the product UI can reasonably convey it directly.
