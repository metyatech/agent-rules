...
- GUI: start by identifying when in the user's real workflow they will open the screen, what they are trying to accomplish at that moment, what context they already have, and what decision or action must become easier right then; design the interface around that moment, with ergonomics, in-app guidance, and a deliberate user journey from current context to next action to result to optional detail, and use persistent explanatory prose only when the user cannot reasonably infer the action from the UI itself.
