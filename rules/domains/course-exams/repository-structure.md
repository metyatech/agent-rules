...
- An assessment directory MUST contain only `assessment.yaml`, `result/`, and optionally `track-map.yaml` when Track-published. It MUST NOT keep question Markdown locally.
- Assessment-local runtime configuration files such as `assessment-run.json` are not supported.
